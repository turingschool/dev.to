...
RSpec.describe EmailDigest, type: :labor do
  ...
  describe "::send_periodic_digest_email" do
    context "when there's article to be sent" do
      ...
      it "send digest email when there's atleast 3 hot articles" do
        ...
        expect(DigestMailer).to have_received(:digest_email).with(user, [instance_of(Article), instance_of(Article), instance_of(Article)])
      end
    end
  end
end
